        ...
        switch ($order->payment_method) {
            case 'wechat':
                ...
                app('wechat_pay')->refund([
                    ...
                    'refund_fee' => $order->total_amount * 100, // 要退款金额，单位 分
                    // 微信支付的退款结果并不是实时返回的，而是通过退款回调来通知的，
                    // 因此，这里的因此这里需要配上退款回调接口地址
                ...
                break;
            case 'alipay':
                // 用我们刚刚写的方法来生成 一个退款订单号
                $refundNo = Order::getAvaibleRefundNo();
            ...
            default:
                ...
                throw new InterExeption('未知订单支付方式：'.$order->payment_method);
                break;
        }
    }
